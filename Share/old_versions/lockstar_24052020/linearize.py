...
class LinearizeWindow(tk.Toplevel):
    ...
    def calibrate(self):
        ...
        PD_Voltage = self.root.parent.MCU.pop_read_queue()
    ...
    def calculate(self):
        ...
        self.pivots_voltage = [root_scalar(lambda x: (interpolation(x)-self.ZERO)/self.SPAN-fraction, bracket=[self.AOM_voltage[self.big_span[0]], self.AOM_voltage[self.big_span[1]]]).root for fraction in [0.001]+list(np.linspace(1.0/LIN_NUMBER_PIVOTS, 1, LIN_NUMBER_PIVOTS-1))]
    ...
    def accept(self):
        # save calibration
        self.root.parent.Config.SetLinPivots(np.transpose(np.array([self.pivots_voltage, self.ZERO+np.linspace(0,1,LIN_NUMBER_PIVOTS)*self.SPAN])), datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
