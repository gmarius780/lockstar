...
class ControlGUI(tk.Tk):
    ...
    def add_pid_box(self, x, y, pid_id=1):
        pid_box = PIDBox(self, x, y, pid_id)
    ...
    def add_scan_box(self, x, y, analysis_function, waypoint_function):
        scan_box = ScanBox(self, x, y, analysis_function, waypoint_function)
...
class PIDBox:
    def __init__(self, parent, x, y, pid_id=1):
        self.pid_id = pid_id
        self.parent = parent
        # pid frame
        ...
        # buttons
        tk.Button(pidframe, text="Auto...", command=self.auto_pid_settings, font=('Copperplate Gothic Bold', fontsize), foreground='white', background='black').grid(row=4, column=0)
...
class ScanBox:
    def __init__(self, parent, x, y, lockpoints_function, waypoints_function):
        ...
        tk.Button(plotframe, text="Scan Now", command=self.scan, font=('Copperplate Gothic Bold', fontsize), foreground='white', background='black').grid(row=4, column=0, columnspan=3)
    ...
    def calculate_lockpoints(self):
        ...
        self.lockpoints, self.flanks, self.extrainformation = self.lockpoints_function(self.voltage, self.signal1, self.signal2, flank=FLANK[self.Config.lock_flank.get()], sens=float(self.Config.lock_relsens.get()))
