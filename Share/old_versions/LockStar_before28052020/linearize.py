...
class LinearizeWindow(tk.Toplevel):
    ...
    def calculate(self):
        ...
        self.pivots_voltage = [root(lambda x: (interpolation(x)-self.ZERO)/self.SPAN-fraction, bracket=[self.AOM_voltage[self.big_span[0]], self.AOM_voltage[self.big_span[1]]]).x for fraction in [0.001]+list(np.linspace(1.0/LIN_NUMBER_PIVOTS, 1, LIN_NUMBER_PIVOTS-1))]
