...
class Settings:
    ...
    def Save(self):
        fil = open(filename, 'w')    
        fil.write('\n'.join([self.name.get(), self.ADC_Setting.get(), self.DAC_Setting.get(), self.kp.get(), self.ki.get(), self.kd.get(), self.scope_time.get(), self.lin_pivots.get(), self.lin_timestamp.get()]))
        fil.close()
