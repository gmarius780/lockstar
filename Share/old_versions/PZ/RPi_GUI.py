...
class ControlGUI(tk.Tk):
    ...
    def changeScopeTime(self, time):
        ...
        self.plotrange = np.linspace(0, time, SCOPE_POINTS)
    ...
    def setup_plots(self):
        ...
        self.plotrange = np.linspace(0,scope_time,SCOPE_POINTS)
        # fig
    ...
    def setup_lin_box(self):
        ...
        tk.Label(linframe, text='AOM Linearization', anchor=tk.W, font=('Broadway', 18), foreground='white', background='black').pack()
        # make figure
        ...
        self.graph_lin.get_tk_widget().pack()
        # button
        tk.Button(linframe, text="Setup...", command=self.linearize, font=('Copperplate Gothic Bold', self.fontsize), foreground='white', background='black').pack()
        if self.Config.lin_pivots.get() == 'None':
            self.ax_lin.text(0.5, 0.5, "No calibration yet", ha='center', va='center', c='C3')
            self.Config.SetLinPivots(np.array([[np.tan(y-np.pi/2)/10.0+1.8 , y] for y in np.linspace(0.1, np.pi-0.1, LIN_NUMBER_PIVOTS)]), datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
        else:
