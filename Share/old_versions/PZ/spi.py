...
class MCU_Handler:
    ...
    def transfer(self, output=None, sendbytes=SEND_BYTES, readbytes=READ_FLOATS):
        # write data
        if sendbytes>0:
            if output==None:
                output = DataPackage()
        ...
        # read data
        try:
            spiinput = self.spi.readbytes(readbytes*4)
            values = unpack(str(READ_FLOATS)+'f', bytes(spiinput))
            self.append_read_queue(list(values))
        except:
            print("Cannot transfer to MCU: spidev not installed!")
            ...
            if self.mode == "run" or len(self.write_queue)>0:
                ...
                try:
                    ...
                    if len(self.write_queue)>0:
                        self.spi.transfer(self.pop_write_queue())
                    else:
                        self.spi.transfer()
                    print("Got data!")
