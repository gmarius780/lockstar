...
class ControlGUI(tk.Tk):
    ...
    def memory_test_read(self):
        ...
        while not self.MCU.get_GPIO_pin():
            pass
        # read data from MCU
        spiinput = self.MCU.spi.readbytes(16)
        values = unpack('4I', bytes(spiinput))
        print(values)
    ...
    def setup_plots(self):
        ...
        tk.Button(plotframe, text="Find Lines", command=self.calculate_lockpoints, font=('Copperplate Gothic Bold', self.fontsize), foreground='white', background='black').grid(row=9, column=0, columnspan=3)
        
        tk.Button(plotframe, text="Lock", command=self.testrc, font=('Copperplate Gothic Bold', self.fontsize), foreground='white', background='black').grid(row=10, column=0, columnspan=1)
        tk.Button(plotframe, text="Unlock", command=self.memory_test_write, font=('Copperplate Gothic Bold', self.fontsize), foreground='white', background='black').grid(row=10, column=1)
        #tk.Button(plotframe, text="Log", command=self.log, font=('Copperplate Gothic Bold', self.fontsize), foreground='white', background='black').grid(row=10, column=2)
    ...
    def calculate_lockpoints(self):
        ...
        self.flanks = analyzescan.get_flanks(self.piezo, self.errors, float(self.Config.lock_relsens.get()), flank=FLANK[self.Config.lock_flank.get()])
        print("flanks done")
        self.lockpoints = analyzescan.get_lock_points_from_flanks(self.piezo, self.errors, self.flanks, float(self.Config.lock_offset.get()))
        print("lockpoints done")
        self.scan_plot.mark(self.lockpoints)
        self.scan_plot.plot_lock_points(self.piezo, self.flanks, self.lockpoints)
        print(analyzescan.generate_waypoints(self.piezo, self.errors, self.piezo[-1], self.lockpoints[marked,0], FLANK[self.Config.lock_flank.get()], tolerance, self.flanks[marked,1]-self.flanks[marked,0]))
    ...
    def detailed_trace(self):
        ...
        for i in range(2000):
            ...
            while not self.MCU.get_GPIO_pin():
                pass
            spiinput = self.MCU.spi.readbytes(4000)
        ...
        print(transm[:20])
        print(errors[:20])
        self.scan_plot.plot(np.linspace(-200,200,1000000), transm, errors)
        self.piezo = np.linspace(-200,200,1000000)
    ...
    def go_to_lock(self):
        ...
        DP.addUInt8(1 if self.Config.lock_flank.get()=='RISING' else 0)
        ...
        for waypoint in waypoints:
            DP.addFloat(float(waypoint[0]))
            DP.addFloat(float(waypoint[1]))
        self.MCU.transfer(sendbytes=8*len(waypoints), output=DP, readfloats=0)
    ...
    def log(self):
        ...
        while len(self.MCU.read_queue)>0:
            self.MCU.pop_read_queue()
        # get log
        self.MCU.transfer(sendbytes=0, readfloats=4000)
        np.savetxt("daclog.txt", self.MCU.pop_read_queue())
    ...
    def setup_live_box(self):
        ...
        #self.ax_live.set_ylim(-10,10)
        #self.ax_live2.set_ylim(-10,10)
        # buttons
