...
class LinearizeWindow(tk.Toplevel):
    ...
    def calculate(self):
        ...
        self.small_span = [int(tenpercent-distance), int(tenpercent+0.3*distance)]
        self.big_span = [int(tenpercent-distance), int(ninetypercent+distance)]
        
        self.ZERO = np.mean(self.filtered[:self.small_span[0]])
        LIMIT = float(self.clipmax.get())*MAX
        self.SPAN = LIMIT-self.ZERO
        
        interpolation = interp1d(self.AOM_voltage, self.filtered)
        
        self.pivots_voltage = [root_scalar(lambda x: (interpolation(x)-self.ZERO)/self.SPAN-fraction, bracket=[self.AOM_voltage[self.big_span[0]], self.AOM_voltage[self.big_span[1]]]).root for fraction in [0.001]+list(np.linspace(0.01, 1, 100))]
    ...
    def accept(self):
        self.root.Config.SetLinPivots(np.transpose(np.array([self.pivots_voltage, self.ZERO+np.linspace(0,1,101)*self.SPAN])), datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
    ...
    def setup_window(self):
        ...
        # info
        tk.Label(frame, text="Choose at which power you want to clip the calibration (it gets very flat towards 100\%).\nThen tweak the parameters of the Savitzky-Golay filter to get good agreement between raw and filtered data.", font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black').pack()
        
        # filter setup
        filterframe = tk.Frame(frame)
        filterframe.pack()
        tk.Label(filterframe, text="clip max: ", font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black').pack(side=tk.LEFT)
        tk.Spinbox(filterframe, from_=0.00, to=1.00, increment=0.01, borderwidth=0, textvar=self.clipmax, width=4, font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black', buttonbackground='black').pack(side=tk.LEFT)
        tk.Label(filterframe, text=" Savitzky-Golay Filter Window: ", font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black').pack(side=tk.LEFT)
        tk.Spinbox(filterframe, from_=1, to=501, increment=2, borderwidth=0, textvar=self.sg_wl, width=3, font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black', buttonbackground='black').pack(side=tk.LEFT)
        tk.Label(filterframe, text=" Poly Order: ", font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black').pack(side=tk.LEFT)
        tk.Spinbox(filterframe, from_=1, to=20, increment=1, borderwidth=0, textvar=self.sg_po, width=2, font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black', buttonbackground='black').pack(side=tk.LEFT)
        
        # info
        tk.Label(frame, text="\nYou can tweak the zero position of the AOM with these two buttons. This is a bit ambiguous.\nNote that there will be linear interpolation between this zero point and the 1\% pivot.", font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black').pack()
        
        # zero position
        zeroframe = tk.Frame(frame)
        zeroframe.pack()
        tk.Button(zeroframe, text="Shift Zero down", command=(lambda: self.shift_zero(False)), font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black').pack(side=tk.LEFT)
        tk.Button(zeroframe, text="Shift Zero up", command=(lambda: self.shift_zero(True)), font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black').pack(side=tk.LEFT)
        
        # info
        tk.Label(frame, text="\nWhen you are happy with the calibration, click Accept.", font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black').pack()
        
        # cancel / accept 
        buttonframe = tk.Frame(frame)
        buttonframe.pack()
        tk.Button(buttonframe, text="Cancel", command=self.close, font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black').pack(side=tk.LEFT)
        tk.Button(buttonframe, text="Accept", command=self.accept, font=('Copperplate Gothic Bold', self.root.fontsize), foreground='white', background='black').pack(side=tk.RIGHT)
