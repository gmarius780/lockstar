...
class LinearizeWindow(tk.Toplevel):
    ...
    def accept(self):
        ...
        #self.root.parent.Config.SetLinPivots(np.transpose([self.pivots_voltage, self.ZERO+np.linspace(0,1,LIN_NUMBER_PIVOTS)*self.SPAN]), datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
