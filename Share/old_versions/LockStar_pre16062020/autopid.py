...
class AutoPIDWindow(tk.Toplevel):
    ...
    def accept(self):
        ...
        DP.addFloat(0.0)
        DP.addUInt32(10)
        DP.addFloat(self.best_kp)
        DP.addFloat(self.best_ki)
        DP.addFloat(self.best_kd)
        self.root.parent.MCU.transfer(output=DP, sendbytes=4013, readfloats=0)
        # save settings
        self.root.parent.Config.kp.set(self.kp)
        self.root.parent.Config.ki.set(self.ki)
        self.root.parent.Config.kd.set(self.kd)
        # wait for ready flag from MCU
        while self.root.parent.MCU.get_GPIO_pin():
            pass
        # restart live stream
