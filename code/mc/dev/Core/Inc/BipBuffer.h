...
public:
  BipBuffer()
      : pBuffer(NULL), ixa(0), sza(0), ixb(0), szb(0), buflen(0), ixResrv(0),
        szResrv(0) {}
  ...
  bool AllocateBuffer(int buffersize = 4096) {
    ...
    buffersize =
        ((buffersize + si.dwPageSize - 1) / si.dwPageSize) * si.dwPageSize;

    pBuffer =
        (BYTE *)::VirtualAlloc(NULL, buffersize, MEM_COMMIT, PAGE_READWRITE);
    if (pBuffer == NULL)
      return false;
  ...
  //
  // Returns:
  //   BYTE*                    pointer to the first contiguous block, or NULL
  //   if empty.
