...
void RingBuffer_LockRegion(RingBuffer *buf, uint16_t start, uint16_t len) {
  if (buf->lockedRegionsCount < MAX_LOCKED_REGIONS) {
    buf->lockedRegions[buf->lockedRegionsCount].start = start;
    buf->lockedRegions[buf->lockedRegionsCount].len = len;
    buf->lockedRegionsCount++;
  }
}
