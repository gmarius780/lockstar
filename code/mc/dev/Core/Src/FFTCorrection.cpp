...
float FFTCorrection::CalculateCorrection(float pv) {
  ...
  for (int i = 0; i < runningFFT.size(); i++) {
    ...
    float real = coeff.real() * runningFFT[i].fft.real() -
                 coeff.imag() * runningFFT[i].fft.imag() + coeff.real() * e;
    float imag = coeff.imag() * runningFFT[i].fft.real() +
                 coeff.real() * runningFFT[i].fft.imag() + coeff.imag() * e;
    ...
    // pvOut -= amplitude * (runningFFT[i].supportSin * runningFFT[i].fft.real()
    // + runningFFT[i].supportCos * runningFFT[i].fft.imag());
    float temp2 =
        amplitude * (runningFFT[i].supportSin * runningFFT[i].fft.real() +
                     runningFFT[i].supportCos * runningFFT[i].fft.imag());
  }
...
void FFTCorrection::GenerateFFTBasis() {
  ...
  for (int i = 0; i < resonantPeaks.size(); i++) {
    ...
    for (int j = peak_k - int(supportingFreqRange / 2);
         j < peak_k + int(supportingFreqRange / 2); j++) {
      float freq = j * fftStep;
      float fft = 0.0;
      float x = 2 * M_PI * resonantPeaks[i].freq * batchEndTime +
                M_PI * (0.5 + batchEndTime * (freq - resonantPeaks[i].freq));
