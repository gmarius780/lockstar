...
bool lockparameters::CheckLock() {
  ...
  if (this->prevprevvariance == 0)
    res = true;
  if (abs(this->variance - this->prevprevvariance) <
      (max(this->variance, this->prevprevvariance) * (variancetol)))
    res = true;
...
float lockparameters::LockStep() {
  ...
  if (this->reset) {
    ...
    (this->damping) ? (this->integrator = Damping(this->counter))
                    : (this->integrator += output * Ki);
    this->DAC_Slow->WriteInt(this->DAC_Slow->Float2Int(
        this->DAC_Slow->Int2Float(this->currentvalue) + this->integrator));
    ...
    this->Step();
    if (this->cmax
            ? ((err - this->offset) > (cwp - this->offset) * (1 - peaktol))
            : ((err - this->offset) < (cwp - this->offset) * (1 - peaktol))) {
