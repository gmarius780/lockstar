...
#ifdef OLD
float *RecordTrace(ADC_Dev *ADC_DEV, uint8_t ADC_Channel, DAC_Dev *DAC_DEV,
                   uint16_t Steps) {
  ...
  uint32_t output = HalfRange; // we are in two's complement, the lowest output
                               // corresponds to mid-way
...
float *RecordTrace(ADC_Dev *ADC_DEV, bool ReadChannel1, bool ReadChannel2,
                   DAC_Dev *DAC_DEV, float From, float To, uint32_t Steps) {
  ...
  for (uint32_t index = 0; index < Steps; index++) {
    ...
    if (ReadChannel1 && ReadChannel2) {
      ...
      data[index] = ReadChannel1 ? ADC_DEV->Channel1->GetFloat()
                                 : ADC_DEV->Channel2->GetFloat();
    // change target output
    output += StepSize;
  }
...
void Move2Voltage(DAC_Dev *DAC_DEV, float Voltage, float MaxStep) {
  ...
  uint32_t NumberSteps =
      (uint32_t)(((Voltage > Start) ? (Voltage - Start) : (Start - Voltage)) /
                     MaxStep +
                 1);
...
void Go2Lock(DAC_Dev *DAC_DEV, ADC_Dev *ADC_DEV, std::queue<Waypoint> waypoints,
             float Step) {
  ...
  while (!waypoints.empty()) {
    ...
    volatile float input =
        (minmax == WAYPOINT_ERRORS_LARGER || minmax == WAYPOINT_ERRORS_SMALLER)
            ? ADC_DEV->Channel2->GetFloat()
            : ADC_DEV->Channel1->GetFloat();
    if (minmax * input > minmax * value)
      waypoints.pop();
  }
}

__attribute__((section("sram_func"))) void DigitalOutHigh() {
  DigitalOut_GPIO_Port->BSRR = DigitalOut_Pin << 16U;
}

__attribute__((section("sram_func"))) void DigitalOutLow() {
  DigitalOut_GPIO_Port->BSRR = DigitalOut_Pin;
}

void RecordDetailedTrace(ADC_Dev *ADC_DEV, bool ReadChannel1, bool ReadChannel2,
                         DAC_Dev *DAC_DEV, RaspberryPi *RPi, uint32_t From,
                         uint32_t NumberSteps, uint32_t BlockSize) {
  uint32_t BufferSize =
      (ReadChannel1 && ReadChannel2) ? 4 * BlockSize : 2 * BlockSize;
  uint32_t Range = 1048576UL; // 2 ^ 20
  ...
  for (uint32_t Counter = 0; Counter < NumberSteps; Counter++) {
    ...
    if (ReadChannel1 && ReadChannel2) {
      ...
      data[ArrayIndex] = ReadChannel1 ? ADC_DEV->Channel1->GetFloat()
                                      : ADC_DEV->Channel2->GetFloat();
      ArrayIndex++;
    }
...
void OptimizePID(ADC_Dev *ADC_DEV, uint8_t ADC_Channel, DAC_Dev *DAC_DEV,
                 RaspberryPi *RPi, PID *PIDLoop) {
  ...
  while (true) {
    ...
    for (counter = 0; counter < 1000; counter++) {
      ...
      while (!ADC_DEV->isReady())
        ;
      input = ADC_Channel == 1 ? ADC_DEV->Channel1->GetFloat()
                               : ADC_DEV->Channel2->GetFloat();
      // save reading
      trace[counter] = input;
      // apply PID
...
void SelfTest(ADC_Dev *ADC_DEV, DAC_Dev *DAC_1, DAC_Dev *DAC_2,
              RaspberryPi *RPi) {
  // turn_LED6_on();
  ...
  while (!DAC_1->isReady())
    ;
  DAC_1->MakeDMA2Way();
  DAC_1->ReadControlReg_Step1();
  while (!DAC_1->isReady())
    ;
  countdown = 5;
  while (countdown > 0)
    countdown--;
  control_reg = DAC_1->ReadControlReg_Step2();
  while (!DAC_1->isReady())
    ;
  ...
  while (!DAC_2->isReady())
    ;
  // DAC_2->MakeDMA2Way();
  DAC_2->ReadControlReg_Step1();
  while (!DAC_2->isReady())
    ;
  control_reg = DAC_2->ReadControlReg_Step2();
  while (!DAC_2->isReady())
    ;
