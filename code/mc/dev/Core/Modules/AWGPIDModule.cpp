...
/**
 * User can upload buffers containing setpoints for both channels. The module
 * will then be a pid controller for both outputs and use the buffer as
 * setpoints with a sampling rate, set by the user
 */
class AWGPIDModule : public BufferBaseModule {
  static const uint32_t BUFFER_LIMIT_kBYTES =
      140; // if this is chosen to large (200) there is no warning, the MC
...
public:
  ...
  __attribute__((optimize(0))) // Otherwise the AWGPID Module hangs here and
                               // doesn't execute the main loop
                               void
                               run() {
    ...
    while (true) {
      while (this->locked and (is_output_on or is_output_ttl)) {
        ...
        this->dac_1->write(this->pid_one->calculate_output(
            this->setpoint_one, adc->channel1->get_result(), dt));
        this->dac_2->write(this->pid_two->calculate_output(
            this->setpoint_two, adc->channel2->get_result(), dt));
      }
    }
  }

  void handle_rpi_input() {
    if (BufferBaseModule::handle_rpi_base_methods() ==
