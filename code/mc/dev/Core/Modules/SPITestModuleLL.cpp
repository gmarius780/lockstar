...
public:
...
#ifdef LOOPBACK_MODE
    if (BufferCmp((uint8_t *)SPI_MASTER_TxBuffer,
                  (uint8_t *)SPI_MASTER_RxBuffer, BUFFER_SIZE)) {
  ...
  void SPI_TX_Callback(SPI_TypeDef *SPIx) {
    ...
    if (SPI_MASTER == SPIx)
      if (SPI_MASTER_DIRECTION == LL_SPI_FULL_DUPLEX ||
          SPI_MASTER_DIRECTION == LL_SPI_SIMPLEX_TX)
        LL_SPI_TransmitData8(SPI_MASTER,
                             SPI_MASTER_TxBuffer[SPI_MASTER_TXIDX++]);

#ifdef SPI_SLAVE
    if (SPI_SLAVE == SPIx)
      if (SPI_SLAVE_DIRECTION == LL_SPI_FULL_DUPLEX ||
          SPI_SLAVE_DIRECTION == LL_SPI_SIMPLEX_TX)
  ...
  void SPI_RX_Callback(SPI_TypeDef *SPIx) {
    ...
    if (SPI_MASTER == SPIx)
      if (SPI_MASTER_DIRECTION == LL_SPI_FULL_DUPLEX ||
          SPI_MASTER_DIRECTION == LL_SPI_SIMPLEX_RX)
        SPI_MASTER_RxBuffer[SPI_MASTER_RXIDX++] =
            LL_SPI_ReceiveData8(SPI_MASTER);

#ifdef SPI_SLAVE
    if (SPI_SLAVE == SPIx)
      if (SPI_SLAVE_DIRECTION == LL_SPI_FULL_DUPLEX ||
          SPI_SLAVE_DIRECTION == LL_SPI_SIMPLEX_RX)
  ...
  void SPI_DMA_EOT_Callback(SPI_TypeDef *SPIx) {
    ...
    while (
        LL_DMA_GetDataLength(SPI_MASTER_RXDMA, SPI_MASTER_RXDMA_STREAM) != 0 ||
        LL_SPI_IsActiveFlag_RXWNE(SPIx) || LL_SPI_GetRxFIFOPackingLevel(SPIx)) {
    }
    LL_DMA_DisableStream(SPI_MASTER_RXDMA, SPI_MASTER_RXDMA_STREAM);

    while (LL_DMA_IsEnabledStream(SPI_MASTER_TXDMA, SPI_MASTER_TXDMA_STREAM) ||
           LL_DMA_IsEnabledStream(SPI_MASTER_RXDMA, SPI_MASTER_RXDMA_STREAM)) {
    }
  ...
  void SPI_config(SPI_TypeDef *SPIx, uint32_t SPIx_DIRECTION, uint32_t NSS_MODE,
                  bool isSlave) {
...
#endif
#ifdef DMA_MODE
    LL_DMA_ConfigAddresses(SPI_MASTER_TXDMA, SPI_MASTER_TXDMA_STREAM,
                           (uint32_t)SPI_MASTER_TxBuffer,
                           LL_SPI_DMA_GetTxRegAddr(SPI_MASTER),
                           LL_DMA_DIRECTION_MEMORY_TO_PERIPH);
    LL_DMA_SetDataLength(SPI_MASTER_TXDMA, SPI_MASTER_TXDMA_STREAM,
                         BUFFER_SIZE);
    LL_DMA_ConfigAddresses(SPI_MASTER_RXDMA, SPI_MASTER_RXDMA_STREAM,
                           LL_SPI_DMA_GetRxRegAddr(SPI_MASTER),
                           (uint32_t)SPI_MASTER_RxBuffer,
                           LL_DMA_DIRECTION_PERIPH_TO_MEMORY);
    LL_DMA_SetDataLength(SPI_MASTER_RXDMA, SPI_MASTER_RXDMA_STREAM,
                         BUFFER_SIZE);

    LL_DMA_SetStreamPriorityLevel(SPI_MASTER_TXDMA, SPI_MASTER_TXDMA_STREAM,
                                  LL_DMA_PRIORITY_LOW);
    LL_DMA_SetStreamPriorityLevel(SPI_MASTER_RXDMA, SPI_MASTER_RXDMA_STREAM,
                                  LL_DMA_PRIORITY_LOW);
    ...
    LL_DMA_SetFIFOThreshold(SPI_MASTER_TXDMA, SPI_MASTER_TXDMA_STREAM,
                            LL_DMA_FIFOTHRESHOLD_FULL);
    LL_DMA_SetFIFOThreshold(SPI_MASTER_RXDMA, SPI_MASTER_RXDMA_STREAM,
                            LL_DMA_FIFOTHRESHOLD_FULL);
