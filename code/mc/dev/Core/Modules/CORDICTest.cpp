...
#define ANGLE_LIB                                                              \
  (int32_t)0x08000000 /* pi/8 in arm math lib input angle mapping */
...
public:
  CORDICTestModule() {
    ...
    LL_CORDIC_Config(
        CORDIC, LL_CORDIC_FUNCTION_ARCTANGENT, /* cosine function */
  ...
  void run() {
    ...
    *pCalculatedSin = to_float(CORDIC->RDATA, 133.3 * 4, 4);
    ...
    for (uint32_t i = 0; i < ARRAY_SIZE; i++) {
      if (Check_Residual_Error(aCalculatedSin[i], sin_values[i], ERROR) ==
          FAIL) {
        Error_Handler();
      }
    }
  ...
  void sampling_timer_interrupt() {
    // float value = ((float)*(dacPointer++) / (float)(1 <<
...
__STATIC_INLINE float to_float(int32_t value, uint32_t scaling_factor,
                               uint32_t offset) {
  return ((float)value / (float)(1 << FIXED_POINT_FRACTIONAL_BITS)) *
             scaling_factor +
         offset;
}
/******************************
