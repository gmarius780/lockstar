...
#define ETRP_CLK                                                               \
  10000000 /* external source Clock frequency after the divider in Hz*/
#define TREF                                                                   \
  ((ETRP_CLK) / (MEASURED_SIGNAL))    /* the ideal number of count between two \
                                         input capture */
...
#define TIM1_DMAR_ADDRESS                                                      \
  ((uint32_t)0x4001004c) /* TIM DMAR address for burst access*/
#define BUFFER_DATA_NUMBER ((uint32_t)9)

// uint32_t aSRC_Buffer[BUFFER_DATA_NUMBER] = {4000 - 1, 1, 0, 800, 10000 - 1,
// 0, 0, 8500, 4000 - 1, 2, 0, 2000};
uint32_t aSRC_Buffer[BUFFER_DATA_NUMBER] = {
    5000 - 1, 0, 1000, 10000 - 1, 0, 7000, 1000 - 1, 0, 500};
uint32_t chunke_times_buffer[10] = {5000,  8000, 3000, 4000, 8000,
                                    12000, 3000, 5000, 5000, 2000};
...
public:
  ...
  void run() {
    ...
    // Bursts because 4 consecutive register are written (TIM1_ARR, TIM1_RCR
    // TIM1_CCR1, TIM1_CCR2)
    ...
    LL_TIM_ConfigDMABurst(TIM1, LL_TIM_DMABURST_BASEADDR_CCR3,
...
static void PPM_Calculate(void) {
  ...
  for (i = 0; i < PPM_BUFFER_SIZE; i++) {
    aPPMBuffer[i] =
        (TREF - ((aCaptureBuffer[i + 1] - aCaptureBuffer[i]))) * CONSTANT;
  }
}
