...
public:
  ...
  void handle_rpi_input() {
    ...
    // rpi->send_package(((uint32_t*)rpi->get_read_buffer())[0],
    // ((uint32_t*)rpi->get_read_buffer())[1]);
  }
  ...
  void method_two(RPIDataPackage *read_package) {
    ...
    write_package->push_to_buffer<uint32_t>(
        read_package->pop_from_buffer<uint32_t>());
    write_package->push_to_buffer<uint32_t>(
        read_package->pop_from_buffer<uint32_t>());
...
//	/*if(GPIO_Pin == DigitalIn_Pin && HAL_GPIO_ReadPin(DigitalIn_GPIO_Port,
//DigitalIn_Pin)==GPIO_PIN_RESET){ 		locking = true; 		PIDLoop->Reset();
//		PIDLoop2->Reset();
...
//	if(GPIO_Pin == DigitalIn_Pin && HAL_GPIO_ReadPin(DigitalIn_GPIO_Port,
//DigitalIn_Pin)==GPIO_PIN_SET){ 		locking = false; 		turn_LED6_off();
//	}*/
//
//}
