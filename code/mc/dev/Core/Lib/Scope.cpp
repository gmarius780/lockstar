...
Scope::Scope(ADC_Device *adc, LinearizableDAC *dac_1, LinearizableDAC *dac_2) {
  ...
  setup = adc_active_mode = sample_in_one = sample_in_two = sample_out_one =
...
bool Scope::setup_scope(uint32_t sampling_prescaler,
                        uint32_t sampling_counter_max, bool sample_in_one,
                        bool sample_in_two, bool sample_out_one,
...
bool Scope::sample() {
  // if buffer_index >= buffer_length: push_buffers_to_rpi_data_package has to
  // be called first
...
bool Scope::push_buffers_to_rpi_data_package(RPIDataPackage *data_package,
                                             uint32_t buffer_offset,
                                             uint32_t package_size) {
  if (setup and buffer_index > 0 and
      buffer_offset + package_size <= buffer_length) {
    if (sample_in_one) {
      ...
      for (uint32_t i = 0; i < package_size; i++)
        if (buffer_offset + i < this->buffer_index)
          data_package->push_to_buffer<float>(
              buffer_out_one[buffer_offset + i]);
        else
          data_package->push_to_buffer<float>(0.);
    }
    if (sample_out_two) {
      for (uint32_t i = 0; i < package_size; i++)
        if (buffer_offset + i < this->buffer_index)
          data_package->push_to_buffer<float>(
              buffer_out_two[buffer_offset + i]);
        else
          data_package->push_to_buffer<float>(0.);
    }

    if (buffer_offset + package_size ==
        buffer_length) // the whole buffer has been read
      buffer_index = 0;
    return true;
  } else {
    return false;
...
bool Scope::set_sampling_rate(uint32_t sampling_prescaler,
