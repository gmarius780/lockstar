...
__attribute__((section(".BDMABlock"))) __attribute__((__aligned__(32)))
...
DAC1_Device::DAC1_Device(DAC_Device_TypeDef *DAC_conf) : DAC_Device(DAC_conf) {
  ...
  DAC_conf->BDMA_InitStruct->PeriphOrM2MSrcAddress =
      (uint32_t) & (DAC_conf->SPIx->TXDR);
  DAC_conf->BDMA_InitStruct->MemoryOrM2MDstAddress = (uint32_t)dma_buffer;
  DAC_conf->BDMA_InitStruct->NbData = DAC1_BUFFER_SIZE;
  LL_BDMA_Init(DAC_conf->BDMAx, __LL_BDMA_GET_CHANNEL(DAC_conf->BDMA_Channelx),
               DAC_conf->BDMA_InitStruct);
...
DAC2_Device::DAC2_Device(DAC_Device_TypeDef *DAC_conf) : DAC_Device(DAC_conf) {
  ...
  LL_DMA_Init(DAC_conf->DMAx, __LL_DMA_GET_STREAM(DAC_conf->DMA_Streamx),
              DAC_conf->DMA_InitStruct);
  // EnableIT_TC(DAC_conf->DMA_Streamx);
  LL_SPI_EnableIT_EOT(DAC_conf->SPIx);
}

__attribute__((section(".itcmram"))) void DAC_Device::write(float output) {
  // while (busy)
  // {
  //     skipped++;
  // }
...
__attribute__((section(".itcmram"))) void DAC_Device::write() {
  // while (busy)
  // {
  //     skipped++;
  // }
  ...
  // dma_buffer[0] = byteBuffer[0];
  // dma_buffer[1] = byteBuffer[1];
  // dma_buffer[2] = byteBuffer[2];
  // byteBuffer.pop(3);
  begin_dma_transfer();
...
void DAC_Device::config_output() {
   ...
   * configuration. Based on this the output range of the DAC_Device is set
   */
...
float DAC_Device::get_max_output() { return this->max_output; }

float DAC_Device::get_last_output() { return this->last_output; }
