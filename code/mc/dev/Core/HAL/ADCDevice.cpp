...
ADC_Device::ADC_Device(ADC_Device_TypeDef *ADC_conf, uint8_t ch1_config,
                       uint8_t ch2_config) {
  ...
  ADC_conf->DMA_InitStructRx->PeriphOrM2MSrcAddress =
      (uint32_t) & (ADC_conf->SPIx->RXDR);
  ADC_conf->DMA_InitStructRx->MemoryOrM2MDstAddress = (uint32_t)ADCdma_buffer;

  ADC_conf->DMA_InitStructTx->PeriphOrM2MSrcAddress =
      (uint32_t) & (ADC_conf->SPIx->TXDR);
  ADC_conf->DMA_InitStructTx->MemoryOrM2MDstAddress =
      (uint32_t)adc_config_buffer;

  LL_DMA_Init(ADC_conf->DMARx, __LL_DMA_GET_STREAM(ADC_conf->DMA_StreamRx),
              ADC_conf->DMA_InitStructRx);
  LL_DMA_Init(ADC_conf->DMATx, __LL_DMA_GET_STREAM(ADC_conf->DMA_StreamTx),
              ADC_conf->DMA_InitStructTx);

  EnableIT_TC(ADC_conf->DMA_StreamRx);
}

ADC_Device_Channel::ADC_Device_Channel(ADC_Device *parentDevice,
                                       uint16_t channelID, uint8_t config) {
...
ADC_Device::dma_transmission_callback(void) {
  ADC_conf->dmaTx_clr_flag(ADC_conf->DMATx);
}
__attribute__((section(".itcmram"))) void
ADC_Device::dma_receive_callback(void) {
