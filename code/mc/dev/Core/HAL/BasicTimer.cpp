...
BasicTimer::BasicTimer(uint8_t timer_x, uint32_t auto_reload,
                       uint32_t prescaler) {
  switch (timer_x) {
  case 2:
    tim_regs = TIM2;
  ...
  // ARPE: Auto-reload preload enable - enable chaning of sampling rate on the
  // fly
...
void BasicTimer::enable_interrupt() {
  tim_regs->EGR |=
      TIM_EGR_UG; // manually trigger update event, otherwise interrupt will be
                  // triggered immediately after enable
  reset_interrupt();
  tim_regs->DIER |= (1 << 0);
}
