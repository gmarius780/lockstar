...
public:
  ...
  // The RPI sends one byte (0-255) via spi to the MC. This value times
  // READ_NBR_BYTES_MULTIPLIER is interpreted als the number of bytes, the MC
  // has to expect from the rpi in this data package
...
private:
  ...
  BasicTimer *comm_reset_timer; // resets is_communicating after an one periode
                                // of this timer
  // COMMUNICATION RESET FREQUENCY= INTERNAL_CLOCK_FREQUENCY/prescaler *
  // counter_max = 275e6/55e3/30000 = 1/12 Hz EVERY COMMUNICATION MUST HAPPEND
  // IN UNDER 12 seconds! This means that the time between sending of a command
  // by the RPI and the polling ACK/NACK by the RPI must be small than 12
  // seconds
  ...
  bool is_communicating; // true after spi interrupt was fired until dma
