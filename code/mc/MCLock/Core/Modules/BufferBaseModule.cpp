...
/***
 * read the method-identifier (integer identifying the method e.g. METHOD_OUTPUT_OFF) and call the appropriate
 * method. If the baseModule doesn't recognize the method-identifier, it returns false such that the child-class
 * deals with it.
 *
...
/**
 * set lenths of the two buffer arrays and length of the two chunks arrays. A chunk allows the user to
 * subdivide the buffer into pieces and output them separately.
 */
...
/**
 * Expects prescaler and counter_max from the datapackage to set the sampling rate:
 * sampling-frq = clock_frq (90e6 Hz) / prescaler / counter_max
 */
...
/**
 * Reads in the chunks for channel one. The number of chunks has to be set beforehand by calling the method initialize_buffers
 */
...
/**
 * Reads in the chunks for channel two. The number of chunks has to be set beforehand by calling the method initialize_buffers
 */
...
/**
 * Reads in an array of length <chunks_size> into the chunk_memory at <ch_chunks> from <read_package>
 */
void BufferBaseModule::set_ch_chunks(RPIDataPackage* read_package, uint32_t chunks_size, uint32_t *ch_chunks) {
...
/**
 * Starts the output of the next chunk on both outputs, if there is currently still a chunk being output, nothing happens.
 * This method has the same effect as a trigger-input if the output_ttl method has previously been called
 */
...
/**
 * enables ttl: on rising edge at the digital input the next chunk will be output
 */
...
/**
 * Reads in the output-buffer values from the rpi for either buffer_one or buffer_two
 * first: reads a uint32_t from <read_package> to determine nbr_values_to_read
 * second: reads <nbr_values_to_read> into <current_read> which is the current buffer position of either channel one or two
 */
void BufferBaseModule::set_ch_buffer(RPIDataPackage* read_package, float *current_read, float *channel_buffer, float *buffer_end, bool buffer_one) {
	this->turn_output_off();
...
/**
 * Reset all the pointers to their start values such that at the next output_on/trigger the first chunks will be output
 */
...
/**
 * If there is currently no chunk being output, it sets the pointers such that the next chunk is starting to be sampled
 */
