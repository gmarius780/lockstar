...
class MCU_Handler:
    def __init__(self, GUI, speed=1000000, verbose=True):
        self.mode = "run"
        self.GUI = GUI
        # read queue
    ...
    def transfer(self, output=None, sendbytes=SEND_BYTES, readfloats=READ_FLOATS):
        ...
        if sendbytes>0:
            ...
            try:
                self.spi.writebytes2(output.bytes)
            except:
    ...
    def transfer_bytes(self, output=None, sendbytes=0, readbytes=0):
        # write data
        if sendbytes>0:
            if output==None:
                output = DataPackage()
